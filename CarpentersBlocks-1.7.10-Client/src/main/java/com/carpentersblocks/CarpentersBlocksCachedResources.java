...
public class CarpentersBlocksCachedResources extends DummyModContainer
{
	...
	public void init()
	{
		...
		((SimpleReloadableResourceManager) Minecraft.getMinecraft().getResourceManager()).reloadResourcePack(FMLClientHandler.instance().getResourcePackFor(this.MODID));
	}
	...
	private void createZip(String dir, String fileName) throws Exception
	{
		...
		{
			out.putNextEntry(new ZipEntry("assets/" + this.MODID.toLowerCase() + object[this.RESOURCE_PATH] + ".png"));
			ImageIO.write((BufferedImage) object[this.RESOURCE_IMAGE], "png", out);
			out.closeEntry();
		}
		...
		{
			String resourcePrefix = "assets/" + this.MODID.toLowerCase();
			for (Object[] object : resources)
			{
				out.putNextEntry(new ZipEntry(resourcePrefix + object[this.RESOURCE_PATH] + ".png"));
				ImageIO.write((BufferedImage) object[this.RESOURCE_IMAGE], "png", out);
				out.closeEntry();
			}
		}
