...
public class GarageDoor extends AbstractMultiBlock implements ISided
{
	...
	{
		return this.getRigidity(TE) == DOOR_RIGID;
	}
	...
	class DoorPieceDistanceComparator implements Comparator<TEBase>
	{
		...
		public int compare(TEBase tileEntity1, TEBase tileEntity2)
		{
			...
			double dist1 = player.getDistance(tileEntity1.xCoord, tileEntity1.yCoord, tileEntity1.zCoord);
			double dist2 = player.getDistance(tileEntity2.xCoord, tileEntity2.yCoord, tileEntity2.zCoord);
	...
	/**
	 * When passed a TEBase, will play state change sound if piece is nearest to
	 * player out of all connecting door pieces.
	 * <p>
	 * The server would normally handle this and send notification to all nearby
	 * players, but sound source will be dependent on each player's location.
	 *
	...
	public void playStateChangeSound(TEBase TE)
	{
		...
		List<TEBase> list = new ArrayList<>(set); // For sorting
		...
		if (list.get(0).equals(TE))
			TE.getWorldObj().playAuxSFXAtEntity(null, 1003, TE.xCoord, TE.yCoord, TE.zCoord, 0);
	}

	/**
	 * Helper function for determining properties based on a nearby garage door
	 * piece around the given coordinates.
	 *
	...
	public TEBase findReferencePiece(World world, int x, int y, int z, ForgeDirection axis)
	{
		...
		do
		{
			...
			if (temp1 != null && this.getDirection(temp1).equals(axis))
				return temp1;
			if (temp2 != null && this.getDirection(temp2).equals(axis))
				return temp2;
		}
		// TODO gamerforEA add condition [0]
	...
	/**
	 * Copies relevant properties and owner from source tile entity to
	...
	{
		return this.getState(TE) == STATE_OPEN;
	}
	 ...
	 * <p>
	 * If a block is open and not in the topmost position, it cannot be selected
	 * or collided with.
	 *
	 * @param TE the {@link TEBase}
	...
	public ForgeDirection[] getLocateDirs(TEBase TE)
	{
		...
		return new ForgeDirection[] { ForgeDirection.UP, ForgeDirection.DOWN, dirPlane, dirPlane.getOpposite() };
	}
