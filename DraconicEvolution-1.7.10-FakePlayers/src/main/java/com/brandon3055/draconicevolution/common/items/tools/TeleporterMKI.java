...
public class TeleporterMKI extends ItemDE
{
	...
	{
		TeleportLocation loc = this.getLocation(stack);
		if (loc == null)
		{
		...
		if (player.getHealth() > 2 || player.capabilities.isCreativeMode)
		{
			...
			if (EventConfig.teleporterBlackList.contains(entity))
				return true;
			if (player != entity && EventUtils.cantDamage(player, entity))
				return true;
			if (EventConfig.teleporterCheckRegion && EventUtils.cantBreak(player, loc.getXCoord(), loc.getYCoord(), loc.getZCoord()))
				return true;
			...
			loc.sendEntityToCoords(entity);
			if (player.worldObj.isRemote)
				player.addChatMessage(new ChatComponentText(new ChatComponentTranslation("msg.teleporterSentMob.txt").getFormattedText() + " x:" + (int) loc.getXCoord() + " y:" + (int) loc.getYCoord() + " z:" + (int) loc.getZCoord() + " Dimension: " + loc.getDimensionName()));
		}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean extraInformation)
	{
		...
		else
		{
			...
			list.add(EnumChatFormatting.WHITE + "{x:" + (int) ItemNBTHelper.getDouble(stack, "X", 0) + " y:" + (int) ItemNBTHelper.getDouble(stack, "Y", 0) + " z:" + (int) ItemNBTHelper.getDouble(stack, "Z", 0) + " Dim:" + this.getLocation(stack).getDimensionName() + "}");
