...
public class WeatherController extends BlockCustomDrop
{
	...
	{
		this.icon_top = iconRegister.registerIcon(References.RESOURCESPREFIX + "weather_controller/weather_controller_top");
		this.icon_bottom = iconRegister.registerIcon(References.RESOURCESPREFIX + "machine_side");
		this.icon_side_rain_off = iconRegister.registerIcon(References.RESOURCESPREFIX + "weather_controller/weather_controller_rain");
		this.icon_side_rain_on = iconRegister.registerIcon(References.RESOURCESPREFIX + "weather_controller/weatherController_rain_active");
		this.icon_side_thunder_off = iconRegister.registerIcon(References.RESOURCESPREFIX + "weather_controller/weatherController_thunderstorm");
		this.icon_side_thunder_on = iconRegister.registerIcon(References.RESOURCESPREFIX + "weather_controller/weatherController_thunderstorm_active");
		this.icon_side_sun_off = iconRegister.registerIcon(References.RESOURCESPREFIX + "weather_controller/weatherController_sun");
		this.icon_side_sun_on = iconRegister.registerIcon(References.RESOURCESPREFIX + "weather_controller/weatherController_sun_active");
	}
		...
		if (tile != null && tile instanceof TileWeatherController)
			if (((TileWeatherController) tile).charges > 0)
				if (((TileWeatherController) tile).mode == 0)
					icon_side = this.icon_side_sun_on;
				else if (((TileWeatherController) tile).mode == 1)
					icon_side = this.icon_side_rain_on;
				else
					icon_side = this.icon_side_thunder_on;
			else if (((TileWeatherController) tile).mode == 0)
				icon_side = this.icon_side_sun_off;
			else if (((TileWeatherController) tile).mode == 1)
				icon_side = this.icon_side_rain_off;
			else
				icon_side = this.icon_side_thunder_off;
	...
	{
		if (!world.isRemote)
			// TODO gamerforEA use EntityPlayer.openGui
			player.openGui(DraconicEvolution.instance, GuiHandler.GUIID_WEATHER_CONTROLLER, world, x, y, z);
		return true;
	}
