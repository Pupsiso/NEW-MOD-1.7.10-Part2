...
public class Generator extends BlockCustomDrop
{
	...
	{
		this.icon_front = iconRegister.registerIcon(References.RESOURCESPREFIX + "animated/generator_front_active");
		this.icon_front_inactive = iconRegister.registerIcon(References.RESOURCESPREFIX + "generator_front");
		this.icon_side = iconRegister.registerIcon(References.RESOURCESPREFIX + "machine_side");
		this.icon_back = iconRegister.registerIcon(References.RESOURCESPREFIX + "animated/machine_fan");
		this.icon_back_inactive = iconRegister.registerIcon(References.RESOURCESPREFIX + "machine_fan");
		for (int i = 0; i < 4; i++)
		{
			this.icon_top[i] = iconRegister.registerIcon(References.RESOURCESPREFIX + "machine_top_" + i);
		}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float prx, float pry, float prz)
	{
		if (!world.isRemote)
			...
			player.openGui(DraconicEvolution.instance, GuiHandler.GUIID_GENERATOR, world, x, y, z);
		return true;
	}
