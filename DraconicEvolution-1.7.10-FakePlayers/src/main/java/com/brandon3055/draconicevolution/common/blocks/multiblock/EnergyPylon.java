...
{//todo fix sphere renderer
	...
	{
		this.icon_input = iconRegister.registerIcon(References.RESOURCESPREFIX + "energy_pylon_input");
		this.icon_output = iconRegister.registerIcon(References.RESOURCESPREFIX + "energy_pylon_output");
		this.icon_active_face = iconRegister.registerIcon(References.RESOURCESPREFIX + "energy_pylon_active_face");
	}
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side)
	{
		...
		if (thisTile == null)
			return this.icon_input;
		return !thisTile.reciveEnergy ? this.icon_output : this.icon_input;
	}
	...
	public void onNeighborBlockChange(World world, int x, int y, int z, Block block)
	{
		...
		TileEnergyPylon thisTile = tile instanceof TileEnergyPylon ? (TileEnergyPylon) tile : null;
		if (meta == 0)
		{
		...
		else if (thisTile == null || meta == 1 && !this.isGlass(world, x, y + 1, z) || meta == 2 && !this.isGlass(world, x, y - 1, z))
			world.setBlockMetadataWithNotify(x, y, z, 0, 2);
		if (thisTile != null)
		{
			// TODO gamerforEA code replace, old code:
			// thisTile.onActivated();
			boolean prevValue = thisTile.findCoresNearbyCheck;
			try
			{
				thisTile.findCoresNearbyCheck = true;
				thisTile.onActivated();
			}
			finally
			{
				thisTile.findCoresNearbyCheck = prevValue;

			}
			// TODO gamerforEA code end
		}

		if (world.getBlockMetadata(x, y, z) == 0 && tile != null)
	...
	public int getComparatorInputOverride(World world, int x, int y, int z, int meta)
	{
		...
		if (tile != null)
			return (int) (tile.getEnergyStored() / tile.getMaxEnergyStored() * 15D);
		return 0;
	}
}
