...
public abstract class MiningTool extends ToolBase implements IUpgradableItem
{
	...
	public boolean breakAOEBlocks(ItemStack stack, int x, int y, int z, int breakRadius, int breakDepth, EntityPlayer player)
	{
		...
		if (block != null)
			...
			{
				if (block.isToolEffective(s, meta) || this.func_150893_a(stack, block) > 1F)
					effective = true;
			}
	...
	protected void breakExtraBlock(ItemStack stack, World world, int x, int y, int z, int totalSize, EntityPlayer player, float refStrength, boolean breakSound, Map<Block, Integer> blockMap)
	{
		...
		if (block.getMaterial() instanceof MaterialLiquid || block.getPlayerRelativeBlockHardness(player, world, x, y, x) <= 0 && !player.capabilities.isCreativeMode)
		...
		if (!world.isRemote)
		{
			...
			if (block.removedByPlayer(world, player, x, y, z, true))
			{
				...
				if (xp < 0)
					xp = block.getExpDrop(world, meta, EnchantmentHelper.getFortuneModifier(player));
				if (xp > 0)
	...
	{
		// TODO gamerforEA code start
		if (!EventConfig.enableDigSpeedUpgrade)
	...
	public List<String> getUpgradeStats(ItemStack stack)
	{
		...
		for (ItemConfigField field : this.getFields(stack, 0))
		{
			...
			else if (field.name.equals(References.DIG_DEPTH))
				depth = (Integer) field.max;
			else if (field.name.equals(References.ATTACK_AOE))
				attackaoe = 1 + (Integer) field.max * 2;
		}

		strings.add(InfoHelper.ITC() + StatCollector.translateToLocal("gui.de.RFCapacity.txt") + ": " + InfoHelper.HITC() + Utills.formatNumber(this.getMaxEnergyStored(stack)));
		...
		strings.add(InfoHelper.ITC() + StatCollector.translateToLocal("gui.de.max.txt") + " " + StatCollector.translateToLocal("gui.de.DigSpeed.txt") + ": " + InfoHelper.HITC() + this.getEfficiency(stack));
		if (attackaoe > 0)
