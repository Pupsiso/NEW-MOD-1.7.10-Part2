...
public class DissEnchanter extends BlockCustomDrop
{
	...
	{
		this.blockIcon = iconRegister.registerIcon(References.RESOURCESPREFIX + "dissEnchanter_side");
		this.top = iconRegister.registerIcon(References.RESOURCESPREFIX + "dissEnchanter_top");
		this.bottom = iconRegister.registerIcon(References.RESOURCESPREFIX + "dissEnchanter_bottom");
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float prx, float pry, float prz)
	{
		if (!world.isRemote)
			...
			player.openGui(DraconicEvolution.instance, GuiHandler.GUIID_DISSENCHANTER, world, x, y, z);
		return true;
	}
