...
public class DraconiumChest extends BlockCustomDrop
{
	...
	{
		for (String s : STORAGE_BLACKLIST)
		{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float cx, float cy, float cz)
	{
		...
		if (player.isSneaking())
		{
			...
			if (world.isRemote)
				player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.DARK_GREEN + "" + StatCollector.translateToLocal("msg.draconiumChestEditmode.txt") + EnumChatFormatting.DARK_AQUA + " " + te.editMode));
			if (te.editMode)
			{
				if (world.isRemote)
		...
		if (te.editMode && (this.isDye(player.getHeldItem(), "dyeRed") || this.isDye(player.getHeldItem(), "dyeGreen") || this.isDye(player.getHeldItem(), "dyeBlue")))
		{
			int increment = cy > 0.5F ? 5 : -5;

			if (this.isDye(player.getHeldItem(), "dyeRed"))
			{
				te.red += increment;
				if (te.red > 255)
					te.red = 255;
			...
			if (this.isDye(player.getHeldItem(), "dyeGreen"))
			{
				te.green += increment;
				if (te.green > 255)
					te.green = 255;
			...
			if (this.isDye(player.getHeldItem(), "dyeBlue"))
			{
				te.blue += increment;
				if (te.blue > 255)
					te.blue = 255;
		...
		if (!world.isRemote)
			...
			player.openGui(DraconicEvolution.instance, GuiHandler.GUIID_DRACONIC_CHEST, world, x, y, z);
		return true;
	}
	...
	{
		this.blockIcon = par1IconRegister.registerIcon(References.RESOURCESPREFIX + "draconiumChest");
	}
