...
public class PlayerDetectorAdvanced extends BlockCustomDrop
{
	...
	{
		this.side_inactive = iconRegister.registerIcon(References.RESOURCESPREFIX + "advanced_player_detector_side_inactive");
		this.side_active = iconRegister.registerIcon(References.RESOURCESPREFIX + "advanced_player_detector_side_active");
		this.top = iconRegister.registerIcon(References.RESOURCESPREFIX + "machine_top_0");
		this.bottom = iconRegister.registerIcon(References.RESOURCESPREFIX + "machine_side");
	}
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side)
	{
		...
		else
		{
			if (detector != null && detector.output)
				side_icon = this.side_active;
			else
				side_icon = this.side_inactive;
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int p_149727_6_, float p_149727_7_, float p_149727_8_, float p_149727_9_)
	{
		if (!world.isRemote)
			...
			player.openGui(DraconicEvolution.instance, GuiHandler.GUIID_PLAYERDETECTOR, world, x, y, z);
		return true;
	}
