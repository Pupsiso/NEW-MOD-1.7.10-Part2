...
public class EntityEnderArrow extends EntityDraconicArrow
{
	...
	protected void onImpact(Entity entityHit)
	{
		if (this.shootingEntity == null)
			return;
		double startX = this.shootingEntity.posX;
		double startY = this.shootingEntity.posY;
		double startZ = this.shootingEntity.posZ;
		...
		if (!(this.shootingEntity instanceof EntityPlayer) || this.shootingEntity.isDead)
			return;

		if (entityHit != null)
		{
			// TODO gamerforEA code start
			if (!this.fake.cantDamage(entityHit))
		...
		if (!this.worldObj.isRemote)
			...
			{
				EntityPlayerMP shooter = (EntityPlayerMP) this.shootingEntity;

				// TODO gamerforEA code start
				if (shooter.playerNetServerHandler == null)
				{
					this.setDead();
					return;
				}
				...
				if (shooter.playerNetServerHandler.func_147362_b().isChannelOpen() && shooter.worldObj == this.worldObj)
				{
					...
					EnderTeleportEvent event = new EnderTeleportEvent(shooter, x + 0.5, y + 0.5, z + 0.5, travelDmg);
					if (!MinecraftForge.EVENT_BUS.post(event))
					{
						if (this.shootingEntity.isRiding())
							this.shootingEntity.mountEntity(null);
						((EntityPlayer) this.shootingEntity).setPositionAndUpdate(event.targetX, event.targetY, event.targetZ);
						this.shootingEntity.fallDistance = 0.0F;
						this.shootingEntity.attackEntityFrom(DamageSource.fall, (float) (travelDist / 5D));
					}
				}
			}
		this.setDead();
