...
public abstract class BlockModFurnace extends BlockContainer
{
	...
	public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer p, int i, float j, float k, float f)
	{
		...
		if (furnace != null)
		{
			...
			{
				if (!w.isRemote)
					p.openGui(DivineRPG.instance, this.guiID, w, x, y, z);
				return true;
			}
		}
		return false;
					...
					for (int i1 = 0; i1 < entity.getSizeInventory(); ++i1)
					{
						...
						if (itemstack != null)
						{
							...
							while (itemstack.stackSize > 0)
							{
								...
								EntityItem entityitem = new EntityItem(w, (double) ((float) x + f), (double) ((float) y + f1), (double) ((float) z + f2), ModUtils.cloneStackWithSize(itemstack, j1));
