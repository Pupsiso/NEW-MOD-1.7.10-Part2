		...
		if (objPos.typeOfHit == MovingObjectType.BLOCK)
		{
			...
			if (stack.getItem() == ArcanaItems.staffStarlight)
			{
				if (!world.isRemote && ArcanaHelper.getProperties(player).useBar(25))
				{
					for (int i = 0; i < 8; i++)
					{
						EntityStar entityStar = new EntityStar(world, (double) blockX + 0.5D, (double) blockY + 25D, (double) blockZ + 0.5D);

						// TODO gamerforEA code start
						entityStar.fake.setRealPlayer(player);
						// TODO gamerforEA code end

						world.spawnEntityInWorld(entityStar);
					}
