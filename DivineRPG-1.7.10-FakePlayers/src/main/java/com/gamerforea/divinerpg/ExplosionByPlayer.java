...
public final class ExplosionByPlayer extends Explosion
{
	...
	public final void doExplosionA()
	{
		...
		int i0 = MathHelper.floor_double(this.explosionX - this.explosionSize - 1.0D);
		int i1 = MathHelper.floor_double(this.explosionX + this.explosionSize + 1.0D);
		int j0 = MathHelper.floor_double(this.explosionY - this.explosionSize - 1.0D);
		int j1 = MathHelper.floor_double(this.explosionY + this.explosionSize + 1.0D);
		int k0 = MathHelper.floor_double(this.explosionZ - this.explosionSize - 1.0D);
		int k1 = MathHelper.floor_double(this.explosionZ + this.explosionSize + 1.0D);
		List<Entity> entities = this.worldObj.getEntitiesWithinAABBExcludingEntity(this.exploder, AxisAlignedBB.getBoundingBox(i0, j0, k0, i1, j1, k1));
		...
		for (int i = 0; i < entities.size(); ++i)
		{
			...
			if (distance <= 1.0D)
			{
				...
				if (d3 != 0.0D)
				{
					...
					d1 /= d3;
					d2 /= d3;
					double d4 = this.worldObj.getBlockDensity(vec3, entity.boundingBox);
					double d5 = (1.0D - distance) * d4;
					entity.attackEntityFrom(DamageSource.setExplosionSource(this), (int) ((d5 * d5 + d5) / 2.0D * 8.0D * this.explosionSize + 1.0D));
		...
		for (int i = 0; i < 16; ++i)
		{
			for (int j = 0; j < 16; ++j)
			{
				for (int k = 0; k < 16; ++k)
				{
					...
					{
						double d0 = i / 30.0F - 1.0F;
						double d1 = j / 30.0F - 1.0F;
						double d2 = k / 30.0F - 1.0F;
