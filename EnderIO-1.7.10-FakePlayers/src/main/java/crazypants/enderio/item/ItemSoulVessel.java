...
public class ItemSoulVessel extends Item implements IResourceTooltipProvider
{
	...
	protected ItemSoulVessel()
	{
		...
		this.setUnlocalizedName(ModObject.itemSoulVessel.unlocalisedName);
		this.setMaxStackSize(64);
		this.blackList = new ArrayList<String>();
		Collections.addAll(this.blackList, Config.soulVesselBlackList);
	}
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer player, World world, int x, int y, int z, int side, float xOffset, float yOffset, float zOffset)
	{
		...
		if (root.hasKey("isStub"))
		{
			if (mob instanceof EntityLiving)
				((EntityLiving) mob).onSpawnWithEgg(null);
		}
		else
			mob.readFromNBT(root);
		...
		boolean spaceClear = world.checkNoEntityCollision(mob.boundingBox) && world.getCollidingBoundingBoxes(mob, mob.boundingBox).isEmpty();
		if (!spaceClear)
			return false;
	...
	public boolean itemInteractionForEntity(ItemStack item, EntityPlayer player, EntityLivingBase entity)
	{
		...
		if (entity.isDead || !(entity instanceof EntityLivingBase) || EventUtils.cantDamage(player, entity))
			return false;
		// TODO gamerforEA code end
		...
		this.setDisplayNameFromEntityNameTag(capturedMobVessel, entity);
	...
	public ItemStack createVesselWithEntity(Entity ent)
	{
		...
		this.setDisplayNameFromEntityNameTag(res, ent);
		return res;
	}
	...
	{
		NBTTagCompound maxHealthAttrib = this.getAttributeFromStack(item, "generic.maxHealth");
	...
	public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4)
	{
		if (par1ItemStack != null)
		{
			...
			float health = this.getHealthFromStack(par1ItemStack);
			if (health >= 0)
			{
				float maxHealth = this.getMaxHealthFromStack(par1ItemStack);
			...
			String fluidName = this.getFluidNameFromStack(par1ItemStack);
			if (fluidName != null)
			{
			...
			DyeColor color = this.getColorFromStack(par1ItemStack);
			if (color != null)
