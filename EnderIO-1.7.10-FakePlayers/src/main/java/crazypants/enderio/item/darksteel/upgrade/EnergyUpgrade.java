...
public class EnergyUpgrade extends AbstractUpgrade
{
	...
	public EnergyUpgrade(NBTTagCompound tag)
	{
		...
		this.capacity = tag.getInteger(KEY_CAPACITY);
		this.energy = tag.getInteger(KEY_ENERGY);
		this.maxInRF = tag.getInteger(KEY_MAX_IN);
		this.maxOutRF = tag.getInteger(KEY_MAX_OUT);

		// TODO gamerforEA code start
		if (this.capacity > 0 && this.energy > this.capacity)
	...
	public void addDetailedEntries(ItemStack itemstack, EntityPlayer entityplayer, List list, boolean flag)
	{
		...
		upgradeStr.add(EnumChatFormatting.DARK_AQUA + EnderIO.lang.localizeExact(this.getUnlocalizedName() + ".name"));
		SpecialTooltipHandler.addDetailedTooltipFromResources(upgradeStr, this.getUnlocalizedName());

		String percDamage = (int) Math.round(this.getAbsorptionRatio(itemstack) * 100) + "";
	...
	{
		upgradeRoot.setInteger(KEY_CAPACITY, this.capacity);
		upgradeRoot.setInteger(KEY_ENERGY, this.energy);

		upgradeRoot.setInteger(KEY_MAX_IN, this.maxInRF);
		upgradeRoot.setInteger(KEY_MAX_OUT, this.maxOutRF);
	}

	public boolean isAbsorbDamageWithPower(ItemStack stack)
	{
		return RANDOM.nextDouble() < this.getAbsorptionRatio(stack);
	}
	...
	{
		// TODO gamerforEA code start
		if (this.capacity > 0 && energy > this.capacity)
