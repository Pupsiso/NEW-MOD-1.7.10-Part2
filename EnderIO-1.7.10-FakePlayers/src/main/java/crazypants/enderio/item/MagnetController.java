...
public class MagnetController
{
	...
	public void onPlayerTick(TickEvent.PlayerTickEvent event)
	{
		...
		ActiveMagnet activeMagnet = this.getActiveMagnet(player);
		if (activeMagnet != null && player.getHealth() > 0f)
		{
			this.doHoover(player);
			if (event.side == Side.SERVER && player.worldObj.getTotalWorldTime() % 20 == 0)
			{
				ItemMagnet.drainPerSecondPower(activeMagnet.item);
				player.inventory.setInventorySlotContents(activeMagnet.slot, activeMagnet.item);
	...
	private ActiveMagnet getActiveMagnet(EntityPlayer player)
	{
		...
		for (int slot = 0; slot < maxSlot; slot++)
		{
			...
			if (ItemMagnet.isActive(stackInSlot) && ItemMagnet.hasPower(stackInSlot))
				return new ActiveMagnet(stackInSlot, slot);
		}
		return null;
	}
	...
	public void doHoover(EntityPlayer player)
	{
		...
		AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(player.posX - Config.magnetRange, player.posY - Config.magnetRange, player.posZ - Config.magnetRange, player.posX + Config.magnetRange, player.posY + Config.magnetRange, player.posZ + Config.magnetRange);
		List<Entity> interestingItems = this.selectEntitiesWithinAABB(player.worldObj, aabb);
