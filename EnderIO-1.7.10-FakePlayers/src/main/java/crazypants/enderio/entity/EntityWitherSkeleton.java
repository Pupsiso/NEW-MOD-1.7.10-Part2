...
public class EntityWitherSkeleton extends EntitySkeleton
{
	...
	public EntityWitherSkeleton(EntitySkeleton entity)
	{
		...
		for (int i = 0; i < entity.getLastActiveItems().length; i++)
		{
			...
			this.setCurrentItemOrArmor(i, stack);
		}
	}
		...
		if (source.damageType.equals("player"))
		{
			...
			if (stack != null && stack.hasTagCompound())
			{
				...
				if (beheading > 0 && this.worldObj.rand.nextInt(100) < beheading * 10)
					EntityUtil.spawnItemInWorldWithRandomMotion(this.worldObj, new ItemStack(Items.skull, 1, 1), this.posX, this.posY, this.posZ);
			}
		}
		if (source.getEntity() instanceof EntityLivingBase)
		{
			int lootingLevel = EnchantmentHelper.getLootingModifier((EntityLivingBase) source.getEntity());
			if (this.worldObj.rand.nextInt(Math.max(1, 1 - lootingLevel)) == 0)
			{
				Item necroticBone = GameRegistry.findItem("TConstruct", "materials");
				if (necroticBone != null)
					EntityUtil.spawnItemInWorldWithRandomMotion(this.worldObj, new ItemStack(necroticBone, 1, 8), this.posX, this.posY, this.posZ);
			}
		}
	}
}
