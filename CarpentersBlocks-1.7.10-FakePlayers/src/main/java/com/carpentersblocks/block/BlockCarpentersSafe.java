...
public class BlockCarpentersSafe extends BlockCoverable
{
	 ...
	 */
	public static final ItemStack[] upgradeStack = { new ItemStack(Blocks.gold_block), new ItemStack(Blocks.diamond_block), new ItemStack(Blocks.emerald_block) };
	...
	public boolean hasAccess(World world, int x, int y, int z, EntityPlayer player)
	{
		TEBase tile = this.getTileEntity(world, x, y, z);
		return tile == null || this.canPlayerActivate(tile, player);
	}
	 ...
	 */
	@Override
	protected boolean canPlayerActivate(TEBase TE, EntityPlayer player)
	{
		// TODO gamerforEA code start
		if (player instanceof EntityPlayerMP)
		{
			...
			if (netHandler == null || !netHandler.netManager.isChannelOpen())
				return false;
		}
		// TODO gamerforEA code end

		// TODO gamerforEA add condition [2]
		if (PlayerPermissions.hasElevatedPermission(TE, player, true) || EventUtils.hasPermission(player, EventConfig.safeAccessPermission))
			return true;
	 ...
	 */ protected void postOnBlockActivated(TEBase TE, EntityPlayer entityPlayer, int side, float hitX, float hitY, float hitZ, ActionResult actionResult)
	{
		...
		if (!Safe.isOpen(TE) && this.canPlayerActivate(TE, entityPlayer))
		{
			...
			if (entityPlayer.isSneaking())
			{
				...
				if (itemStack != null && isUpgrade(itemStack) && !TE.hasAttribute(TEBase.ATTR_UPGRADE))
				{
					TE.addAttribute(TEBase.ATTR_UPGRADE, itemStack);
	...
	public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune)
	{
		...
		if (TE != null && TE instanceof TECarpentersSafe)
			if (TE.hasAttribute(TEBase.ATTR_UPGRADE))
				ret.add(TE.getAttributeForDrop(TEBase.ATTR_UPGRADE));
	...
	{
		TEBase tileEntity = this.getSimpleTileEntity(world, x, y, z);
