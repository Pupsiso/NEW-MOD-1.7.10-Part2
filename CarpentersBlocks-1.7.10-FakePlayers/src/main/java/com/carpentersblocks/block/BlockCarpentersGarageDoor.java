...
public class BlockCarpentersGarageDoor extends BlockCoverable
{
	...
	private void destroyInDirection(World world, int x, int y, int z, boolean doDrop, ForgeDirection dir)
	{
		...
		if (block.equals(this))
		{
			...
			if (doDrop)
			{
				TEBase temp = this.getTileEntity(world, x, y, z);
	...
	private void create(TEBase TE, World world, int x, int y, int z)
	{
		...
		EntityPlayer player = TE instanceof TECarpentersGarageDoor ? ((TECarpentersGarageDoor) TE).fake.get() : ModUtils.getModFake(world);
		// TODO gamerforEA code end

		// TODO gamerforEA add condition [1]
		for (; y >= 0 && this.canPlaceBlockAt(world, x, y, z); --y)
		{
			// TODO gamerforEA code start
	 ...
	 */ public void onNeighborBlockChange(World world, int x, int y, int z, Block block)
	{
		if (!world.isRemote)
		{
			...
			TEBase TE = this.getTileEntity(world, x, y, z);
			if (TE != null)
				if (data.isHost(TE) && !(this.canPlaceBlockOnSide(world, x, y, z, 0) || world.getBlock(x, y + 1, z).equals(this)))
					this.destroy(world, x, y, z, true);
				else
				{
					...
					if (block != null && block.canProvidePower() && powerState != data.getState(TE))
					{
						...
						if (data.isOpen(TE))
						{
							...
							if (!garageHasPower)
								state = GarageDoor.STATE_CLOSED;
						}
						else
							state = GarageDoor.STATE_OPEN;
	 ...
	 * @param y           Y position
	 * @param z           Z position
	 * @param willHarvest True if Block.harvestBlock will be called after this, if the
	...
	public boolean removedByPlayer(World world, EntityPlayer entityPlayer, int x, int y, int z)
	{
		...
		{
			this.destroy(world, x, y, z, !entityPlayer.capabilities.isCreativeMode);
			return false;
		}
	...
	public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune)
	{
		...
		list.addAll(super.getDrops(world, x, y, z, this.METADATA_DROP_ATTR_ONLY, fortune));
		return list;
	}
	...
	public boolean canPlaceBlockAt(World world, int x, int y, int z)
	{
		if (super.canPlaceBlockAt(world, x, y, z))
			return this.canPlaceBlockOnSide(world, x, y, z, 0) || world.getBlock(x, y + 1, z).equals(this);
		return false;
	}
	 ...
	 */ public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLiving, ItemStack itemStack)
	{
		...
		// TODO gamerforEA code start
		if (TE instanceof TECarpentersGarageDoor && entityLiving instanceof EntityPlayer)
			((TECarpentersGarageDoor) TE).fake.setProfile(entityLiving);
		// TODO gamerforEA code end
		...
		if (tile != null && tile instanceof TEBase)
		{
			...
			switch (axis)
			{
				case UP:
					...
					{
						case WEST:
							newAngle = cbTile.getData() & ~0x70 | ForgeDirection.NORTH.ordinal() << 4;
							break;
						case NORTH:
							newAngle = cbTile.getData() & ~0x70 | ForgeDirection.EAST.ordinal() << 4;
							break;
						case EAST:
							newAngle = cbTile.getData() & ~0x70 | ForgeDirection.SOUTH.ordinal() << 4;
							break;
						case SOUTH:
							newAngle = cbTile.getData() & ~0x70 | ForgeDirection.WEST.ordinal() << 4;
							break;
						default:
							break;
					}
