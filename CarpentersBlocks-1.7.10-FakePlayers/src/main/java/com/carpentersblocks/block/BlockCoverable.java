...
public class BlockCoverable extends BlockContainer
{
	 ...
	 * <p>
	 * Due to the amount of control needed over this, vanilla calls will always
	 * return an invisible icon.
	 ...
	 */ public IIcon getIcon(IBlockAccess blockAccess, int x, int y, int z, int side)
	{
		...
		return block instanceof BlockCoverable ? this.getIcon() : getWrappedIcon(block, blockAccess, x, y, z, side, itemStack.getItemDamage());
	}
	...
	/**
	 * For South-sided blocks, rotates and sets the block bounds using the
	 * provided ForgeDirection.
	 *
	...
	protected void setBlockBounds(float minX, float minY, float minZ, float maxX, float maxY, float maxZ, ForgeDirection dir)
	{
		...
		{
			case DOWN:
				this.setBlockBounds(minX, 1.0F - maxZ, minY, maxX, 1.0F - minZ, maxY);
				break;
			case UP:
				this.setBlockBounds(minX, minZ, minY, maxX, maxZ, maxY);
				break;
			case NORTH:
				this.setBlockBounds(1.0F - maxX, minY, 1.0F - maxZ, 1.0F - minX, maxY, 1.0F - minZ);
				break;
			case EAST:
				this.setBlockBounds(minZ, minY, 1.0F - maxX, maxZ, maxY, 1.0F - minX);
				break;
			case WEST:
	 ...
	 * dropBlockAsItem}, they create a block event with parameters itemId and
	 * metadata, allowing the {@link ItemStack} to be recreated and dropped.
	 *
	...
	{
		int fortune = 1;
		return new ItemStack(this.getItemDropped(metadata, world.rand, fortune), 1, metadata);
	}
	...
	{
		return new TEBase[] { this.getSimpleTileEntity(world, x, y - 1, z), this.getSimpleTileEntity(world, x, y + 1, z), this.getSimpleTileEntity(world, x, y, z - 1), this.getSimpleTileEntity(world, x, y, z + 1), this.getSimpleTileEntity(world, x - 1, y, z), this.getSimpleTileEntity(world, x + 1, y, z) };
	}

	/**
	 * Returns tile entity if block tile entity is instanceof TEBase.
	 * <p>
	 * Used for generic purposes such as getting pattern, dye color, or cover of
	 * another Carpenter's block. Is also used if block no longer exists, such
	 * as when breaking a block and ejecting attributes.
	 */
	...
	/**
	 * Returns tile entity if block tile entity is instanceof TEBase and also
	 * belongs to this block type.
	 */
		...
		if (item instanceof ICarpentersHammer && ((ICarpentersHammer) item).canUseHammer(world, entityPlayer))
		{
			...
			if (!actionResult.altered)
			{
				if (entityPlayer.isSneaking())
					this.popAttribute(TE, effectiveSide);
				else
	...
	{
		if (TE.hasAttribute(TEBase.ATTR_ILLUMINATOR))
			TE.createBlockDropEvent(TEBase.ATTR_ILLUMINATOR);
	 ...
	 */ public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (!actionResult.altered)
			if (PlayerPermissions.hasElevatedPermission(TE, entityPlayer, false))
			{
				...
				if (itemStack != null)
				{
					...
					int effectiveSide = TE.hasAttribute(TEBase.ATTR_COVER[side]) ? side : 6;
					...
					else if (ItemRegistry.enableChisel && itemStack.getItem() instanceof ICarpentersChisel && ((ICarpentersChisel) itemStack.getItem()).canUseChisel(world, entityPlayer))
					{
						...
						if (TE.hasAttribute(TEBase.ATTR_COVER[effectiveSide]))
							if (this.onChiselClick(TE, effectiveSide, false))
								actionResult.setAltered();
					...
					else if (FeatureRegistry.enableCovers && BlockProperties.isCover(itemStack))
					{
						...
						if (effectiveSide == 6 && (!this.canCoverSide(TE, world, TE.xCoord, TE.yCoord, TE.zCoord, 6) || TE.hasAttribute(TEBase.ATTR_COVER[6])))
							effectiveSide = side;

						if (this.canCoverSide(TE, world, TE.xCoord, TE.yCoord, TE.zCoord, effectiveSide) && !TE.hasAttribute(TEBase.ATTR_COVER[effectiveSide]))
						{
						...
						if (FeatureRegistry.enableIllumination && BlockProperties.isIlluminator(itemStack))
						{
							if (!TE.hasAttribute(TEBase.ATTR_ILLUMINATOR))
							{
								TE.addAttribute(TEBase.ATTR_ILLUMINATOR, itemStack);
								actionResult.setAltered().decInventory().setSoundSource(itemStack);
							}
						}
						else if (FeatureRegistry.enableOverlays && BlockProperties.isOverlay(itemStack))
						{
							if (!TE.hasAttribute(TEBase.ATTR_OVERLAY[effectiveSide]) && (effectiveSide < 6 && TE.hasAttribute(TEBase.ATTR_COVER[effectiveSide]) || effectiveSide == 6))
							{
		...
		else
		{
			...
			this.onNeighborBlockChange(world, TE.xCoord, TE.yCoord, TE.zCoord, this);
	...
	public boolean onChiselClick(TEBase TE, int side, boolean leftClick)
	{
		...
		if (design.equals(""))
		{
			...
			TEBase[] TE_list = this.getAdjacentTileEntities(world, TE.xCoord, TE.yCoord, TE.zCoord);
	 ...
	 */ public void onNeighborBlockChange(World world, int x, int y, int z, Block block)
	{
		...
		if (!world.isRemote)
		{
			...
			if (TE != null)
				...
				{
					if (TE.hasAttribute(TEBase.ATTR_COVER[side]))
					{
						...
						if (world.isSideSolid(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, dir.getOpposite()) && this.isSideSolid(world, x, y, z, dir))
							TE.removeAttributes(side);
					}
				}
		}
	 ...
	 */ public int isProvidingStrongPower(IBlockAccess blockAccess, int x, int y, int z, int side)
	{
		...
		if (BlockProperties.hasAttribute(TE, TEBase.ATTR_COVER[effectiveSide]))
		{
		...
		else if (BlockProperties.hasAttribute(TE, TEBase.ATTR_COVER[6]))
		{
	...
	{
		TEBase TE = this.getTileEntity(world, target.blockX, target.blockY, target.blockZ);

		if (TE != null)
		{

			int effectiveSide = TE.hasAttribute(TEBase.ATTR_COVER[target.sideHit]) ? target.sideHit : 6;
			ItemStack itemStack = BlockProperties.getCover(TE, effectiveSide);

			if (BlockProperties.hasAttribute(TE, TEBase.ATTR_OVERLAY[effectiveSide]))
			{
				Overlay overlay = OverlayHandler.getOverlayType(TE.getAttribute(TEBase.ATTR_OVERLAY[effectiveSide]));
	 ...
	 * destroy effects if left-clicked with a Carpenter's Hammer while player is
	 * in creative mode.
	 *
	 ...
	 */ public float getBlockHardness(World world, int x, int y, int z)
	{
		...
		if (BlockProperties.hasAttribute(TE, TEBase.ATTR_COVER[6]))
		{
	...
	@Override
	/**
	 * Currently only called by fire when it is on top of this block. Returning
	 * true will prevent the fire from naturally dying during updating. Also
	 * prevents fire from dying from rain.
	...
	/**
	 * Drops block as {@link ItemStack} and notifies relevant systems of block
	 * removal. Block attributes will drop later in destruction.
	 * <p>
	 * This is usually called when a
	 * {@link #onNeighborBlockChange(World, int, int, int, Block) neighbor
	 * changes}.
	 *
	 * @param world     the {@link World}
	 * @param x         the x coordinate
	...
	{
		// Drop attributes
		int metadata = dropBlock ? 0 : this.METADATA_DROP_ATTR_ONLY;
	 ...
	 */ public boolean removedByPlayer(World world, EntityPlayer entityPlayer, int x, int y, int z, boolean willHarvest)
	{
		...
		int metadata = entityPlayer != null && entityPlayer.capabilities.isCreativeMode ? this.METADATA_DROP_ATTR_ONLY : 0;
		ArrayList<ItemStack> items = this.getDrops(world, x, y, z, metadata, 0);

		// Drop attributes if block destroyed, and no Carpenter's Tool is held by entity
		if (!this.suppressDestroyBlock(entityPlayer) && super.removedByPlayer(world, entityPlayer, x, y, z, willHarvest))
		{
			for (ItemStack item : items)
			{
				this.dropBlockAsItem(world, x, y, z, item);
	...
	public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune)
	{
		...
		if (metadata == this.METADATA_DROP_ATTR_ONLY)
		...
		if (TE != null)
		{
			...
			{
				if (TE.hasAttribute(TEBase.ATTR_COVER[idx]))
					ret.add(TE.getAttributeForDrop(TEBase.ATTR_COVER[idx]));
				if (TE.hasAttribute(TEBase.ATTR_OVERLAY[idx]))
					ret.add(TE.getAttributeForDrop(TEBase.ATTR_OVERLAY[idx]));
				if (TE.hasAttribute(TEBase.ATTR_DYE[idx]))
					ret.add(TE.getAttributeForDrop(TEBase.ATTR_DYE[idx]));
			}
			if (TE.hasAttribute(TEBase.ATTR_ILLUMINATOR))
				ret.add(TE.getAttributeForDrop(TEBase.ATTR_ILLUMINATOR));
		}
	 ...
	 */ public void randomDisplayTick(World world, int x, int y, int z, Random random)
	{
		...
		if (TE != null)
		{
			...
			if (TE.hasAttribute(TEBase.ATTR_OVERLAY[6]))
				if (OverlayHandler.getOverlayType(TE.getAttribute(TEBase.ATTR_OVERLAY[6])).equals(Overlay.MYCELIUM))
					Blocks.mycelium.randomDisplayTick(world, x, y, z, random);
		}
	}

	/**
	 * Determines if this block can support the passed in plant, allowing it to
	 * be planted and grow. Some examples: Reeds check if its a reed, or if its
	 * sand/dirt/grass and adjacent to water Cacti checks if its a cacti, or if
	 * its sand Nether types check for soul sand Crops check for tilled soil
	 * Caves check if it's a solid surface Plains check if its grass or dirt
	 ...
	 * @param y           Y Position
	 * @param z           Z position
	 * @param side        The direction relative to the given position the plant wants
	...
	public boolean canSustainPlant(IBlockAccess blockAccess, int x, int y, int z, ForgeDirection side, IPlantable plantable)
	{
		...
		if (TE != null)
		{
			...
			for (int side1 = 1; side1 < 7; side1 += 5)
			{
				...
				if (TE.hasAttribute(TEBase.ATTR_OVERLAY[side1]))
			...
			switch (plantable.getPlantType(blockAccess, x, y + 1, z))
			{
				...
				case Beach:
					...
					boolean hasWater = blockAccess.getBlock(x - 1, y, z).getMaterial() == Material.water || blockAccess.getBlock(x + 1, y, z).getMaterial() == Material.water || blockAccess.getBlock(x, y, z - 1).getMaterial() == Material.water || blockAccess.getBlock(x, y, z + 1).getMaterial() == Material.water;
	...
	protected boolean isBlockSolid(IBlockAccess blockAccess, int x, int y, int z)
	{
		...
		if (TE != null)
			return !TE.hasAttribute(TEBase.ATTR_COVER[6]) || BlockProperties.toBlock(BlockProperties.getCover(TE, 6)).isOpaqueCube();
		return false;
	}
	 ...
	 * the adjacent block is at the given coordinates. Args: world, x, y, z,
	 * side
		...
		if (y > 0 && y < blockAccess.getHeight())
		{
			...
			if (TE != null)
			{
				...
				if (TE_adj.getBlockType().isSideSolid(blockAccess, x, y, z, side_adj) == TE_src.getBlockType().isSideSolid(blockAccess, x + side_adj.offsetX, y + side_adj.offsetY, z + side_adj.offsetZ, ForgeDirection.getOrientation(side)))
					if (this.shareFaces(TE_adj, TE_src, side_adj, side_src))
					{
						...
						if (!TE_src.hasAttribute(TEBase.ATTR_COVER[6]) && block_adj.getRenderBlockPass() == 0)
							return !block_adj.isOpaqueCube();
						return !TE_src.hasAttribute(TEBase.ATTR_COVER[6]) || block_src.isOpaqueCube() != block_adj.isOpaqueCube() || block_src.getRenderBlockPass() != block_adj.getRenderBlockPass();
					}
			}
		}
	...
	@SideOnly(Side.CLIENT)
	/**
	 * Returns which pass this block be rendered on. 0 for solids and 1 for
	 * alpha.
	...
	{
		return TE_adj.getBlockType().isSideSolid(TE_adj.getWorldObj(), TE_adj.xCoord, TE_adj.yCoord, TE_adj.zCoord, side_adj) && TE_src.getBlockType().isSideSolid(TE_src.getWorldObj(), TE_src.xCoord, TE_src.yCoord, TE_src.zCoord, side_src);
	}
	...
	/**
	 * This method is configured on as as-needed basis. It's calling order is
	 * not guaranteed.
	 */
	...
	/**
	 * Called if cover and decoration checks have been performed but returned no
	 * changes.
	 */
