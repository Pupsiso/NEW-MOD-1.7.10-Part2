...
public class ItemCarpentersTile extends Item
{
	...
	{
		this.setCreativeTab(CarpentersBlocks.creativeTab);
	}
	...
	{
		this.itemIcon = iconRegister.registerIcon(CarpentersBlocks.MODID + ":" + "tile");
	}
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer entityPlayer, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (EventUtils.cantBreak(entityPlayer, x_offset, y_offset, z_offset))
			return false;
		// TODO gamerforEA code end

		ForgeDirection offset_side = this.getOffsetSide(ForgeDirection.getOrientation(side), hitX, hitY, hitZ);
	...
	private ForgeDirection getOffsetSide(ForgeDirection side, float hitX, float hitY, float hitZ)
	{
		...
		boolean center_clicked = hitX > ratio && hitX < invert_ratio && hitZ > ratio && hitZ < invert_ratio || hitX > ratio && hitX < invert_ratio && hitY > ratio && hitY < invert_ratio || hitZ > ratio && hitZ < invert_ratio && hitY > ratio && hitY < invert_ratio;
