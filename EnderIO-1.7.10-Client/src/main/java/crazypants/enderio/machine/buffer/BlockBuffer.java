...
public class BlockBuffer extends AbstractMachineBlock<TileBuffer> implements IFacade
{
	...
	public IIcon getIcon(int blockSide, int blockMeta)
	{
		...
		return blockSide > 1 ? this.textures[MathHelper.clamp_int(blockMeta, 0, this.textures.length - 1)] : super.getIcon(blockSide, blockMeta);
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int blockSide)
	{
		...
		if (te instanceof TileBuffer)
		{
			...
			if (blockSide > 1)
			{
				...
				// TODO gamerforEA code replace, old code:
				// return this.textures[blockMeta];
				return this.textures[MathHelper.clamp_int(blockMeta, 0, this.textures.length - 1)];
				// TODO gamerforEA code end
			}
		}
	...
	public final class PainterTemplate extends BasicPainterTemplate
	{
		...
		public ResultStack[] getCompletedResult(float chance, MachineRecipeInput... inputs)
		{
			...
			target = target.copy();
			target.stackSize = 1;
			return new ResultStack[] { new ResultStack(BlockBuffer.this.createItemStackForSourceBlock(target, Block.getBlockFromItem(paintSource.getItem()), paintSource.getItemDamage())) };
		}
	}
